...
var (
	...
	Format    = flag.BoolP("format", "f", true, "Allows you to skip formating of the output file, by setting it false")
)
...
func main() {
	...
	err = generate(spec, *packageName, *outputPath, *Format)
	if err != nil {
...
func generate(spec *openapi3.T, packageName, outputFilePath string, format bool) error {
	code, err := generateComponents(spec, packageName, !format)
	if err != nil {
	...
	code, err = generateClient(spec, packageName, !format)
	if err != nil {
